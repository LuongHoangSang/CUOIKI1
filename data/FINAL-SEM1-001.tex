...
\begin{ex}
	...
	{\begin{tikzpicture}  
			\begin{axis}[  ultra thick,scale=0.5,
				...
				\addplot [line width=1.5pt, blue, smooth, domain=3:10] {0.6*x};
				\filldraw (axis cs: 4,2.4) circle (2pt) node[above] {(1)};
				\filldraw (axis cs: 10,6) circle (2pt) node[above] {(2)};
...
\begin{ex}
	...
	{\vspace{-0.5cm}\begin{tikzpicture}  
			\begin{axis}[  ultra thick,scale=0.5,
				...
				axis lines=center, 
				grid style={step=1, line width =0.8pt,black, dashed},
				grid=both, %giới hạn ô lưới
				major grid style={line width=0.8pt,black, dashed},
				xlabel=$\xsi{V}{\left(\si{\meter^3}\right)}$, 		ylabel=$\xsi{p}{\left(\si{\kilo\pascal}\right)}$,
				every axis y label/.style={at=(current axis.above origin),anchor=south},  
				every axis x label/.style={at=(current axis.right of origin),anchor=west},  ]
				\addplot [line width=1.5pt, blue, smooth, domain=0.1:0.85] {8/x};  
				\coordinate (O) at (axis cs: 0,0);
			\end{axis}  
...
% ===================================================================
\begin{ex}
	\immini{Làm thí nghiệm khảo sát mối liên hệ giữa thể tích và nhiệt độ của một lượng khí xác định khi giữ áp suất không đổi. Biết áp kết (1) có mức 0 ứng với áp suất khí quyển, đơn vị đo của áp kế là Bar ($\SI{1}{Bar}=\SI{E5}{Pa}$) ; xilanh (2); pit-tông (3) gắn với tay quay (4); hộp chứa nước nóng (5) và cảm biến nhiệt (6). Đổ nước nóng vào hộp chứa cho ngập toàn xilanh. Dịch chuyển xilanh từ từ sao cho số chỉ của áp kế không đổi.}
	{\includegraphics[scale=0.5]{../figs/FINAL_SEM1_001_3}
	}
	Kết quả đo giá trị của phần thể tích chứa khí và nhiệt độ sau mỗi phút như bảng bên.
	\begin{center}
		\begin{tabular}{|M{1.5cm}|M{8cm}|M{7cm}|}
			\hline
			\thead{Lần đo}&\thead{Nhiệt độ khí trong xilanh $t\ \left(\si{\celsius}\right)$}& \thead{Thể tích khí trong xilanh $\xsi{V}{\left(\milli\liter\right)}$}\\
			\hline
			1 & 45 & 75\\
			\hline
			2&41&74\\
	...
	{\True Khi tăng nhiệt độ từ $\SI{32}{\celsius}$ lên $\SI{117}{\celsius}$ thì thể tích khí tăng thêm xấp xỉ $\SI{20}{\ml}$} 
	\loigiai{
		\begin{itemchoice}
			\itemch Đúng
...
\end{ex}
\begin{ex}
	\immini{Một lượng khí lí tưởng ở trạng thái (1) có áp suất $\SI{4.5E5}{Pa}$, thể tích 1,5 lít và nhiệt độ $\SI{25}{\celsius}$. Khối khí này thực hiện một chu trình biến đổi từ trạng thái (1) đến trạng thái (2) (đường biểu diễn là một phần hyberbol) rồi đến trạng thái (3), sau đó trở về trạng thái (1) như hình vẽ.}
